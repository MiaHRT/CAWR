...
@POLICY_REGISTRY.register('cawr_advPER')
class CAWRPolicy(SACPolicy):
    ...
    def _forward_learn(self, actor_data: List[Dict[str, Any]], critic_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        '''
        This function is modified to optimize the Q network, the value network, and the policy network, with diverse policy loss function (L2, L1, Huber, Skew, Flat) and types of priorities (None, Normal, Standard, AW, ODPR, Quantile) for optimizing policy and decoupled resampling
        '''
        loss_dict = {}
        critic_data = default_preprocess_learn(
            critic_data,
            use_priority=False,
        ...
        if len(critic_data.get('action').shape) == 1:
            critic_data['action'] = critic_data['action'].reshape(-1, 1)

        if self._cuda:
            critic_data = to_device(critic_data, self._device)

        self._learn_model.train()
        self._target_model.train()

        value = self._learn_model.forward(critic_data['obs'], mode='compute_value')['value']

        with torch.no_grad():
            target_value = self._target_model.forward(critic_data, mode='compute_q')['q_value']
            if self._twin_critic:
                target_value = torch.min(target_value[0], target_value[1])

        value_data = expectile_regression_data(value, target_value, critic_data['weight'])
        loss_dict['value_loss'], value_error_per_sample = expectile_regression_error(value_data, self._tau)



        q_value = self._learn_model.forward(critic_data, mode='compute_q')['q_value']

        with torch.no_grad():
            if self._forward_learn_cnt > self._anneal_step:
                target_q_value = self._learn_model.forward(critic_data['next_obs'], mode='compute_value')['value']
            else:
                (mu, sigma) = self._learn_model.forward(critic_data['next_obs'], mode='compute_actor')['logit']
                ...
                next_data = {'obs': critic_data['next_obs'], 'action': next_action}
        ...
        if self._twin_critic:
            q_data0 = v_1step_td_data(q_value[0], target_q_value, critic_data['reward'], critic_data['done'], critic_data['weight'])
            loss_dict['critic_loss'], td_error_per_sample0 = v_1step_td_error(q_data0, self._gamma)
            q_data1 = v_1step_td_data(q_value[1], target_q_value, critic_data['reward'], critic_data['done'], critic_data['weight'])
            ...
            q_data = v_1step_td_data(q_value, target_q_value, critic_data['reward'], critic_data['done'], critic_data['weight'])
            loss_dict['critic_loss'], td_error_per_sample = v_1step_td_error(q_data, self._gamma)
        
        critic_adv = target_value - value.detach()

        actor_data = default_preprocess_learn(
            actor_data,
        ...
        if len(actor_data.get('action').shape) == 1:
            actor_data['action'] = actor_data['action'].reshape(-1, 1)

        if self._cuda:
            actor_data = to_device(actor_data, self._device)

        (mu, sigma) = self._learn_model.forward(actor_data['obs'], mode='compute_actor')['logit']
        ...
        with torch.no_grad():
            target_value = self._target_model.forward(actor_data, mode='compute_q')['q_value']
            if self._twin_critic:
                target_value = torch.min(target_value[0], target_value[1])

            value = self._learn_model.forward(actor_data['obs'], mode='compute_value')['value']

            adv = target_value - value
            
            assert len(adv.shape)==1 and len(critic_adv.shape)==1, (adv.shape, critic_adv.shape)
            total_adv = torch.cat((adv, critic_adv))

            advantage_avg = total_adv.mean()
            advantage_median = total_adv.median()
            advantage_std = total_adv.std()
            advantage_min = total_adv.min()
            if self._PER_type == 'ODPR':
                self._min_adv = min(advantage_min, self._min_adv)
            advantage_max = total_adv.max()
            
            advantage = (adv-advantage_avg)/(advantage_std + 1e-8)
            advantage = torch.exp(self._beta * advantage)
            advantage = torch.clamp(advantage, min=-torch.inf, max=self._max_weight)

            critic_advantage = (critic_adv-advantage_avg)/(advantage_std + 1e-8)
            critic_advantage = torch.exp(self._beta * critic_advantage)
            critic_advantage = torch.clamp(critic_advantage, min=-torch.inf, max=self._max_weight)
        ...
        if self._PER_type == 'Normal':
            priority_per_sample = advantage.abs().tolist()+critic_advantage.abs().tolist()
        elif self._PER_type == 'Standard':
            priority_per_sample = torch.exp(total_adv)
            ...
            priority_per_sample = (total_adv-advantage_avg)/(advantage_std + 1e-8)
            priority_per_sample = F.softmax(self._beta * priority_per_sample)*len(priority_per_sample)
            priority_per_sample = priority_per_sample.abs().tolist()
        elif self._PER_type == 'ODPR':
            priority_per_sample = (total_adv-self._min_adv).abs().tolist()
        elif self._PER_type == 'Quantile':
            priority_per_sample = (total_adv-total_adv.quantile(self._PER_tau))/(advantage_std + 1e-8)
        ...
        return {
            ...
            'std_advantage': advantage_std.item(),
            'min_advantage': advantage_min.item(),
            'max_advantage': advantage_max.item(),
            **loss_dict
        }
